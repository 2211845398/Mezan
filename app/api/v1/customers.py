...
async def create_temporary_customer_endpoint(
    ...
    await audit_service.log(session=db, action="customer.temporary_created", resource_type="customer_profile", resource_id=str(customer.id), user_id=current_user.id, request=request)
    await db.commit()
    return {
...
async def complete_onboarding_endpoint(
    ...
    await audit_service.log(session=db, action="customer.onboarding_completed", resource_type="customer_profile", resource_id=str(customer.id), request=request)
    await db.commit()
