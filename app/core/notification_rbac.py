...
ORG_NOTIFICATION_MANAGER_ROLE_CODES: frozenset[str] = frozenset(
    {"OWNER", "ADMIN", "IT_ADMIN", "HR_MANAGER"}
)
...
def is_company_wide_schedule(row: NotificationSchedule) -> bool:
    """Same rule as :func:`is_company_wide_audience`, for persisted schedules."""
    return row.target_role_code is None and row.branch_id is None
