...
- [x] **0.5** Seeding split: production `app/scripts/core_seed.py` (entrypoint + optional `SEED_ON_STARTUP`); manual `app/scripts/dev_seed.py` with extended fixtures in `dev_seed_fixtures.py` (customers, suppliers, POs, POS, HR, payroll).
- [x] **0.6** API list pagination (`items`/`total`/`limit`/`offset`) on suppliers, users, employees, products, purchase orders, sales invoices (terminal day), POS carts (batched load), discounts, invoice scans, payslips; chart-account depth cache (no per-row N+1).
- [x] **0.7** Web list UX for paginated APIs: shared `PaginatedList` + `paginatedParams`, global React Query `placeholderData`, `PageListSkeleton` route loader, server-mode `DataTable` on suppliers/users/employees/POs/products/customers/journal/discounts/payslips/invoice scans; MSW handlers aligned to `{ items, total, limit, offset }`.
...
- [x] **19.2b** Hierarchical CoA seed: `coa_seed_data.COA_SEED_FOREST`, `plant_coa_tree` / `upgrade_coa_skeleton` in [`coa_seed_service.py`](app/services/coa_seed_service.py); `seed_accounting_defaults` plants fresh tree or upgrades legacy flat DB; preserves system posting codes under BS/P&L groups (`GAP-ACC-004`); tests in [`test_coa_seed_structure.py`](tests/test_coa_seed_structure.py).
- [x] **19.2c** Branch/terminal cash accounts: [`branch_accounting_service.py`](app/services/branch_accounting_service.py) provisions `CASH-{branch}` / `POS-{terminal}` leaves under `10100`; `resolve_settlement_account_id` wired through [`document_posting_service.py`](app/services/document_posting_service.py), vouchers, POS expenses; default AR/AP on customer/supplier create (Option B + `customer_id`/`supplier_id` on GL lines); tests in [`test_branch_cash_accounts.py`](tests/test_branch_cash_accounts.py).
- [x] **W-13.1b** CoA admin UI Phase 4: branch-filtered tree via `GET /chart-accounts/by-branch/{id}` with balances, hide-zero, expand/collapse all, delete confirmation modal, bilingual `name_ar`/`name_en` in group/account dialogs and locale-aware tree labels ([`ChartOfAccountsPage.tsx`](web/src/features/accounting/pages/chart-accounts/ChartOfAccountsPage.tsx)).
