...
import { applyArPayment, type PaymentApplicationCreate } from '../../api';
  ...
  const m = useMutation({
    mutationFn: async () => {
      for (const it of items) {
        ...
        await applyArPayment(it.id, body, newIdempotencyKey());
      }
    },
    onSuccess: async () => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t('ar.drawer_title')}</DialogTitle>
        </DialogHeader>
